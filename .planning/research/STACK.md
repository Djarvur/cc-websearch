...
**Domain:** Claude Code plugin -- Node CLI scripts for web search and fetch
...
npm install @perplexity-ai/perplexity_ai duck-duck-scrape turndown @mozilla/readability jsdom cheerio zod commander turndown-plugin-gfm

# Dev dependencies
npm install -D typescript tsx vitest @types/turndown @types/jsdom
```
...
**If Perplexity SDK has breaking changes or bundle size matters:**
...
**If duck-duck-scrape stops working (DDG changes their HTML):**
...
**If jsdom is too heavy for Readability:**
...
The Claude Code plugin system has specific structural requirements confirmed from official docs:
...
├── scripts/
│   ├── websearch.ts         # WebSearch CLI script
│   └── webfetch.ts          # WebFetch CLI script
├── hooks/
...
Key points from official plugin documentation:
...
**Recommended script invocation in SKILL.md:**

```
node --import tsx "${CLAUDE_PLUGIN_ROOT}/scripts/websearch.ts"
```

This avoids a build step by using Node's `--import` flag with tsx. Alternatively, build to JS with esbuild and invoke directly.
...
_Stack research for: Claude Code web search/fetch plugin_
