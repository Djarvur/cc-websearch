...
_slopcheck was not available at research time. All packages are tagged [VERIFIED] because they are well-established, high-star-count, Mozilla/official packages confirmed on the npm registry AND their official source repositories._
...
async function summarize(content: string, userPrompt: string): Promise<string> {
...
export async function fetchWithRedirects(
  ...
  for (let hop = 0; hop <= maxHops; hop++) {
    ...
    if (response.status >= 400) {
      throw new Error(
        `HTTP ${response.status}: ${statusText(response.status)} at ${currentUrl.href}`,
      );
    }
